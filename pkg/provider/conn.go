...
func (p *FlyProvider) waitForDial(targetId string, dialTimeout time.Duration) error {
...
func (p *FlyProvider) getDockerClient(targetId string) (docker.IDockerClient, error) {
	tsnetConn, err := p.getTsnetConn()
	if err != nil {
		return nil, err
	}
